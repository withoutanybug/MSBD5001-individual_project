...
for t in range(line1):
    (X_test[t,2],X_test[t,3])=X_test[t,2].split('/')[2],X_test[t,2].split('/')[1]
...
y_pred=np.mean(y_df,axis=0)
for t in range(line1):
    y_pred[t]=np.exp(np.sqrt(y_pred[t]))
# =============================================================================
...
submission.to_csv('test.csv',index=1,index_label='id',header=1)
